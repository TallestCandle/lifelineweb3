...
import { createUserWithEmailAndPassword, signInWithEmailAndPassword, updateProfile, signInWithPopup, UserCredential } from 'firebase/auth';
...
import { Stethoscope, ArrowLeft, ShieldAlert, Bot, Eye, EyeOff } from 'lucide-react';
  ...
  const handleAuthSuccess = async (userCredential: UserCredential, isNewUser: boolean = false) => {
    ...
    if (isNewUser) {
        if (isDoctorSignupDisabled) {
            await auth.signOut();
            throw new Error("Doctor sign-ups are currently disabled.");
        }
      ...
      <Card className="w-full max-w-md mx-4 relative bg-card/80 backdrop-blur-sm border-primary/20">
        ...
        <CardContent>
          {!isLogin && isDoctorSignupDisabled && (
           ...
           <Button variant="outline" className="w-full mb-4" onClick={handleGoogleSignIn} disabled={isLoading || (!isLogin && isDoctorSignupDisabled)}>
